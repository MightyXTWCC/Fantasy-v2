...
function adjustPlayerValue(player) {
    const { value, // current value in dollars (e.g., 75000)
...
async function calculatePoints(stats, playerPosition, roundId, playerId) {
    let points = 0;
    // Batting points
    ...
    try {
        ...
        for (const rule of bonusRules) {
            ...
            if (targetPositions.includes('All') || targetPositions.includes(playerPosition)) {
                ...
                if (conditions.min_runs && stats.runs < conditions.min_runs)
                    ruleApplies = false;
                if (conditions.max_runs && stats.runs > conditions.max_runs)
                    ruleApplies = false;
                if (conditions.min_wickets && stats.wickets < conditions.min_wickets)
                    ruleApplies = false;
                if (conditions.max_wickets && stats.wickets > conditions.max_wickets)
                    ruleApplies = false;
                if (conditions.min_catches && catches < conditions.min_catches)
                    ruleApplies = false;
                if (conditions.min_sixes && stats.sixes < conditions.min_sixes)
                    ruleApplies = false;
                if (conditions.min_fours && stats.fours < conditions.min_fours)
                    ruleApplies = false;
                if (ruleApplies) {
...
async function updateH2HScores(roundId) {
    try {
...
async function calculateUserScoreForRound(userId, roundId) {
    try {
